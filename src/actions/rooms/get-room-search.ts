...
export const getRoomBySearch = async ({
    page = 1, category, garage, amenities, orderPrice, inAvailable, onSale, searchTerm, orderMostReserved, city
}: PaginationOptions) => {
    ...
    try {
        ...
        const decodedSearchTerm = decodeURIComponent(searchTerm || "").trim().toLowerCase();
        ...
        if (category || garage || amenities || inAvailable || onSale || decodedSearchTerm) {
            whereConditions.AND = [];
            ...
            if (decodedSearchTerm) {
                whereConditions.AND.push({
                    OR: [
                        { title: { contains: decodedSearchTerm, mode: 'insensitive' } }, // Buscar por nombre de la habitación
                        { category: { name: { contains: decodedSearchTerm, mode: 'insensitive' } } }, // Buscar por nombre de la categoría
                        { motel: { title: { contains: decodedSearchTerm, mode: 'insensitive' } } } // Buscar por nombre del motel
                    ]
                });
            }
        }
