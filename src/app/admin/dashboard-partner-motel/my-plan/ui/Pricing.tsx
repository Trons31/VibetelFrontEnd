...
import { Benefit, Plan, SubscriptionPeriod, SubscriptionTier } from '@/interfaces'; // Asegúrate de importar SubscriptionTier
...
interface Props {
    plans: Plan[];
    tokenSession: string;
    currentMotelTier: SubscriptionTier; // Nueva prop para el tier actual del motel
}

export const Pricing = ({ plans, tokenSession, currentMotelTier }: Props) => {
    ...
    const handlePlanSelect = async (plan: Plan) => {
        ...
        try {
            ...
            toast.success(`Has actualizado tu plan a ${plan.tier} (${selectedBillingOption.label})`,
                {
                    duration: 4000
                }
            );
            ...
            console.error(error);
            toast.error(`Error al actualizar el plan a ${plan.tier}`,
                {
                    duration: 4000
                }
            );
    ...
    const renderPlanCard = (plan: Plan, isCurrentPlan: boolean, isPopular?: boolean) => {
        return (
            <div className={clsx(
                "relative flex flex-col py-8 px-6 rounded-3xl shadow-lg transition-all duration-300 h-full",
                {
                    "bg-gray-50 ring-2 ring-blue-500": isCurrentPlan, // Estilos para el plan actual
                    "bg-gray-50 ring-2 ring-red-200 hover:ring-red-600": !isCurrentPlan // Estilos para otros planes
                }
            )}>
                {isPopular && (
                ...
                <h3 className="text-xl font-semibold text-black mb-4">Plan {plan.tier === "BASIC" ? "Start" : plan.tier === "PREMIUM" ? "Plus" : plan.tier === "ENTERPRISE" ? "Élite" : plan.tier}</h3>
                <div className="mb-3 py-4 px-1">
                    <div className="flex">
                        <p className="text-4xl font-bold text-black">{currencyFormat(getPrice(plan.price))}</p>
                        <p className="text-sm md:text-md text-black mt-2">{`/${selectedBillingOption.label.toLowerCase()}`}</p>
                    </div>
                    <p className="text-sm text-black">+ {formatCommission(plan.commissionPercentage)}% comisión por reserva</p>
                </div>
                ...
                <button
                    ...
                    disabled={loadingPlanId !== null || isCurrentPlan} // Deshabilita si es el plan actual
                    className={
                        clsx(
                            "flex justify-center items-center gap-2 w-full font-semibold py-3 px-6 rounded-xl transition-colors duration-300 transform shadow-md",
                            {
                                "bg-blue-600 text-white cursor-not-allowed": isCurrentPlan, // Estilos para el plan actual (si está activo)
                                "bg-red-600 text-white cursor-not-allowed": loadingPlanId === plan.id, // Estilos para el loading
                                "border-2 border-red-600 text-red-600 hover:scale-[1.02] hover:bg-red-600 hover:text-white": loadingPlanId !== plan.id && !isCurrentPlan // Estilos normales
                            }
                        )
                    }
                >
    ...
    return (
        <>
            ...
            <div className="relative isolate bg-white py-5 md:mt-5 sm:py-8">
                <div className="flex justify-center mt-5">
                    <div>
                        <p className='text-xs md:text-sm text-center text-gray-600' >Duración de la suscripción</p>
                        <SelectOption
                ...
                <div className="mt-10 md:mt-10 grid w-full grid-cols-1 items-stretch gap-7 sm:mt-10 sm:gap-y-0 lg:grid-cols-4">
                    ...
                    {freePlan && renderPlanCard(freePlan, currentMotelTier === freePlan.tier)}
                    {basicPlan && renderPlanCard(basicPlan, currentMotelTier === basicPlan.tier)}
                    {premiumPlan && renderPlanCard(premiumPlan, currentMotelTier === premiumPlan.tier, true)} {/* El plan Premium es el "Más popular" */}
                    {enterprisePlan && renderPlanCard(enterprisePlan, currentMotelTier === enterprisePlan.tier)}
