...
import { getTodayReservationsByMotel, traficReservationToday } from '@/actions';
...
export const TodayTableReservation = ({ motelId, totalReservation }: Props) => {
    ...
    const fetchReservations = useCallback(async () => {
        setIsFetching(true);
        const data = await getTodayReservationsByMotel({ motelId, searchQuery, statusFilter, page, limit: 10 });
    ...
    useEffect(() => {
        fetchReservations();
    }, [fetchReservations, searchQuery, date, statusFilter]);

    useEffect(() => {
        const fetchData = async () => {
            const result = await traficReservationToday(motelId);
            setTotalBooking(result.totalReservations);
            setBookingNoInciadas(result.reservationsNoIniciadas);
            setBookingEnEspera(result.reservationsEnEspera);
            setBookingCompleted(result.reservationsCompletadas);
            setBookingCancel(result.reservationsCanceladas);
        };
        fetchData();
                ...
                <div className="mx-auto rounded-md pt-5 mb-10">
                    <div className="grid grid-cols-8 items-center px-5 ">
                        <div className='col-span-6' >
                            <h1 className=" text-2xl font-bold text-gray-900">Gestión de reservas {formatDate(new Date())} </h1>
                            <p className="text-xs  text-gray-500" >Aquí podrás ver todas las reservas que entran en servicio el dia de hoy en tiempo real. <strong>Para cargar más reservas, simplemente desplázate hacia abajo.</strong></p>
                        </div>

                        <div className='col-span-2 flex justify-end' >
                            <div className="w-fit overflow-hidden rounded-xl shadow border-t-4 border-gray-400 bg-white">
                                <div className="py-2 px-2">
                                    <div className="block">
                                        ...
                                        <span className="ml-3 text-sm font-medium inline capitalize">Reservas en total</span>
                                    </div>
