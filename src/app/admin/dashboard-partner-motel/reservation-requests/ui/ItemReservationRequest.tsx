...
export const ItemReservationRequest = ({ reservationsPending }: Props) => {
  ...
  const UpdateStatusReservatin = async (status: boolean) => {
    ...
    try {
      await axios.patch(`${process.env.NEXT_PUBLIC_API_ROUTE}service/reservation/${reservationsPending.id}/status`,
        { isConfirmed: status },
        {
          headers: {
  ...
  return (
    <>
      <div className="relative border-2 bg-gray-100 border-dashed border-blue-600 rounded">
        <div className={
          clsx(
