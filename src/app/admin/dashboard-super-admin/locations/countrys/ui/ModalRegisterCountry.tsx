...
import { CountryApi } from '@/interfaces';
...
export const ModalRegisterCountry = ({ country, accessToken, mode, isOpen, onClose }: Props) => {
    ...
    const generateIsoCode = (name: string): string => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        try {
            ...
            if (mode === "create") {
                payload = {
                    ...data,
                    isoCode: generateIsoCode(data.name),
                    geonameId: generateGeonameId(data.name), // solo aquí
                };
                ...
                toast.success("País registrado correctamente!");
            } else {
                payload = {
                    ...data,
                    isoCode: generateIsoCode(data.name),
                ...
                await axios.patch(
                    `${process.env.NEXT_PUBLIC_API_ROUTE}locations/countries/${country?.geonameId}`,
                    payload,
                    { headers: { Authorization: `Bearer ${accessToken}` } }
                );
            ...
            console.log(error);
            toast.error("No se pudo guardar el país");
            setIsLoading(false);
        }
    };
        ...
        >
            <div className="bg-white md:rounded-lg shadow-lg w-full md:w-1/2 lg:w-1/2 p-6 max-h-full overflow-y-auto">
                <h2 className="text-xl font-semibold mb-4">
                    {mode === "create" ? "Registrar país" : "Actualizar país"}
                </h2>
