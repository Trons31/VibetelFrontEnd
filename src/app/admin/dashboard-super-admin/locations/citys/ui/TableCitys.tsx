'use client';

import React, { useEffect, useState, useMemo } from 'react';
import { PaginationTable, SkeletonTableRoom } from '@/components';
import { CityApi, CountryApi, DepartmentApi } from '@/interfaces';
import { TbBedOff } from 'react-icons/tb';
import { ModalRegisterCity } from './ModalRegisterCity';

interface Props {
  citys: CityApi[];
  departments: DepartmentApi[];
  accessToken: string;
}

export const TableCity = ({ citys, departments, accessToken }: Props) => {
  const [allCitys, setAllCitys] = useState<CityApi[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(5);

  const [selectedCity, setSelectedCity] = useState<CityApi>();
  const [modalMode, setModalMode] = useState<'create' | 'update'>('create');
  const [isModalOpen, setIsModalOpen] = useState(false);

  useEffect(() => {
    if (citys) {
      setAllCitys(citys);
      setIsLoading(false);
    }
  }, [departments]);

  const sortedCitys = useMemo(() => {
    return [...allCitys].sort((a, b) => {
      // Primero ordenar por nombre del departamento
      const depA = a.department?.name || "";
      const depB = b.department?.name || "";
      if (depA < depB) return -1;
      if (depA > depB) return 1;

      // Si son del mismo departamento, ordenar por nombre de ciudad
      const cityA = a.name || "";
      const cityB = b.name || "";
      return cityA.localeCompare(cityB);
    });
  }, [allCitys]);

  const totalPages = useMemo(() => Math.ceil(sortedCitys.length / itemsPerPage), [sortedCitys, itemsPerPage]);

  const paginatedDepartments = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    const end = start + itemsPerPage;
    return sortedCitys.slice(start, end);
  }, [sortedCitys, currentPage, itemsPerPage]);

  const openCreateModal = () => {
    setSelectedCity(undefined);
    setModalMode('create');
    setIsModalOpen(true);
  };

  const openUpdateModal = (city: CityApi) => {
    setSelectedCity(city);
    setModalMode('update');
    setIsModalOpen(true);
  };

  return (
    <>
      <div className="flex justify-end px-2 md:mx-10 mt-10 mb-7">
        <button onClick={openCreateModal} className="btn-primary text-xs md:text-sm">
          Nuevo ciudad
        </button>
      </div>

      <ModalRegisterCity
        city={selectedCity}
        departments={departments}
        accessToken={accessToken}
        mode={modalMode}
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
      />

      <div className="flex justify-end w-full px-2 md:px-8">
        <div className="flex bg-gray-200 rounded-full py-2 px-3 w-fit">
          <p className="font-normal text-xs md:text-sm">
            {allCitys.length} departamentos registrados
          </p>
        </div>
      </div>

      <div className="mx-5 mt-2 overflow-x-auto">
        <table className="min-w-full shadow-sm">
          <thead className="bg-gray-300 border-b">
            <tr>
              <th className="text-xs md:text-sm font-medium text-gray-900 px-6 py-4 text-left">
                ciudad
              </th>
              <th className="text-xs md:text-sm font-medium text-gray-900 px-6 py-4 text-left">
                Departamento
              </th>
              <th className="text-xs md:text-sm font-medium text-gray-900 px-6 py-4 text-left">
                Pais
              </th>
            </tr>
          </thead>
          <tbody className="bg-gray-100">
            {!isLoading &&
              paginatedDepartments.length > 0 &&
              paginatedDepartments.map((city) => (
                <tr
                  key={city.id}
                  onClick={() => openUpdateModal(city)}
                  className="bg-white border-b cursor-pointer transition duration-300 ease-in-out hover:bg-gray-100"
                >
                  <td className="text-left text-xs md:text-base text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {city.name}
                  </td>
                  <td className="text-left text-xs md:text-base text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {city.department.name ?? '-'}
                  </td>
                  <td className="text-left text-xs md:text-base text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    <td>{city.department?.country?.name ?? '-'}</td>
                  </td>
                </tr>
              ))}
          </tbody>
        </table>

        {isLoading && <SkeletonTableRoom />}

        {!isLoading && paginatedDepartments.length === 0 && (
          <div className="flex justify-center px-5 items-center py-10">
            <div className="no-file-found w-full md:w-1/2 flex flex-col items-center justify-center py-8 px-4 text-center bg-gray-200 rounded-lg shadow-md">
              <TbBedOff size={50} className="text-gray-500" />
              <h3 className="text-md md:text-xl font-semibold mt-4 text-black">
                Ningún departamento encontrado
              </h3>
              <p className="text-xs md:text-lg text-gray-700 mt-2">
                Lo sentimos, no hemos podido encontrar departamentos registrados.
              </p>
            </div>
          </div>
        )}

        {!isLoading && allCitys.length > 0 && (
          <PaginationTable
            nameData='ciudades'
            currentPage={currentPage}
            totalItems={allCitys.length}
            onItemsPerPageChange={(n) => { setItemsPerPage(n); setCurrentPage(1); }}
            totalPages={totalPages}
            onPageChange={(p) => setCurrentPage(p)}
            itemsPerPage={itemsPerPage}
          />
        )}
      </div>
    </>
  );
};
