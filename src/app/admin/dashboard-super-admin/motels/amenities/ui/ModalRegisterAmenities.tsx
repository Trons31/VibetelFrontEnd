...
interface Props {
    amenitie?: AmenitiesMotelInfoApi;
    accessToken: string;
    mode: "create" | "update";
    isOpen: boolean;
    onClose: () => void;
...
export const ModalRegisterAmenities = ({ amenitie, accessToken, mode, isOpen, onClose }: Props) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        try {
            if (mode === "create") {
                await axios.post(
                    `${process.env.NEXT_PUBLIC_API_ROUTE}motel/amenities/info`, data,
                    {
                        headers: {
                ...
                toast.success("Comodidad registrada correctamente!");
            } else {
                await axios.patch(
                    `${process.env.NEXT_PUBLIC_API_ROUTE}motel/amenities/info/${amenitie?.id}`, data,
                    {
                        headers: {
                            Authorization: `Bearer ${accessToken}`,
                        },
            ...
            toast.error("No se pudo guardar la comodidad");
            setIsLoading(false);
        }
    };
        ...
        >
            <div className="bg-white md:rounded-lg shadow-lg w-full md:w-1/2 lg:w-1/2 p-6 max-h-full overflow-y-auto">
                <h2 className="text-xl font-semibold mb-4">
                    {mode === "create" ? "Registrar comodidad" : "Actualizar comodidad"}
                </h2>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="grid px-2 grid-cols-1 mt-5 sm:px-0 gap-6">
                        ...
                        <div>
                            ...
                            />
                            {errors.description && (
                                <span className="text-red-500 text-sm">* La descripción es obligatoria</span>
                            )}
                        </div>
                    </div>
