...
interface Props {
    amenities: AmenitiesMotelInfoApi[];
    accessToken: string;
}

export const TableAmenitiesMotel = ({ amenities, accessToken }: Props) => {
    const [allAmenities, setAllAmenities] = useState<AmenitiesMotelInfoApi[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [currentPage, setCurrentPage] = useState(1);
    const [itemsPerPage, setItemsPerPage] = useState(5);

    const [selectedAmenitie, setSelectedAmenitie] = useState<AmenitiesMotelInfoApi>();
    ...
    const updateAmenitie = (amenitie: AmenitiesMotelInfoApi) => {
        setSelectedAmenitie(amenitie);

    }

    const openCreateModal = () => {
        setSelectedAmenitie(undefined);
        setModalMode("create");
        setIsModalOpen(true);
    };

    const openUpdateModal = (amenitie: AmenitiesMotelInfoApi) => {
        setSelectedAmenitie(amenitie);
        setModalMode("update");
        setIsModalOpen(true);
    };
