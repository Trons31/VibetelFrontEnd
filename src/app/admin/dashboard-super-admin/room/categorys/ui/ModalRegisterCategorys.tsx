...
interface Props {
    category?: AmenitiesMotelInfoApi;
    accessToken: string;
    mode: "create" | "update";
    isOpen: boolean;
    onClose: () => void;
...
export const ModalRegisterCategorys = ({ category, accessToken, mode, isOpen, onClose }: Props) => {
    ...
    const onSubmit = async (data: FormInputs) => {
        ...
        try {
            if (mode === "create") {
                await axios.post(
                    `${process.env.NEXT_PUBLIC_API_ROUTE}room/categories`, data,
                    {
                        headers: {
                ...
                await axios.patch(
                    `${process.env.NEXT_PUBLIC_API_ROUTE}room/categories/${category?.id}`, data,
                    {
                        headers: {
                            Authorization: `Bearer ${accessToken}`,
                        },
        ...
        >
            <div className="bg-white md:rounded-lg shadow-lg w-full md:w-1/2 lg:w-1/2 p-6 max-h-full overflow-y-auto">
                <h2 className="text-xl font-semibold mb-4">
                    {mode === "create" ? "Registrar categoria" : "Actualizar categoria"}
                </h2>
