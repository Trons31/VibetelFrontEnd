...
import { IoArrowForwardOutline, IoBedSharp } from 'react-icons/io5';
        ...
        <div className="px-5 mt-4">
            ...
            <div className='flex gap-2 items-center' >
              <FaStar className='h-4 w-4' />
              {
                motel.averageRating > 0 && (
                  <p className='font-bold mt-1 text-md' >
                    {motel.averageRating}
                  </p>
                )
              }
          ...
          <div>
            <div className="flex items-center gap-1 mt-3" >
              <IoBedSharp  className='h-4 w-4 text-gray-600' />
              <span className="text-gray-500 text-sm" >{motel.totalRooms} habitaciones</span>
            </div>
            <div className="flex items-center gap-1 mt-3" >
              <MdLocationPin className='h-4 w-4 text-gray-600' />
              <span className="text-gray-500 text-sm" >{motel.address}, {motel.neighborhood}</span>
            </div>
          </div>
        </div>
          ...
          <ul className="space-y-4 mb-4 mt-3">
            <li>
              <Link href={`/motel/info/${motel.slug}`}>
                <label className="inline-flex items-center justify-between w-full p-5 text-gray-900 bg-white border border-gray-200 rounded-lg cursor-pointer hover:text-red-600 hover:border-red-500">
                  <div className="block">
                    <p className="w-full text-sm md:text-md font-extralight">Informacion</p>
                  </div>
            ...
            <li>
              <Link href={`motels/${motel.slug}`} >
                <label className="inline-flex items-center justify-between w-full p-5 text-gray-900 bg-white border border-gray-200 rounded-lg cursor-pointer hover:text-red-600 hover:border-red-500">
                  <div className="block">
                    <div className="w-full text-sm md:text-md font-extralight">Habitaciones</div>
                  </div>
                  <IoArrowForwardOutline className='h-4 w-4' />
                </label>
              </Link>
