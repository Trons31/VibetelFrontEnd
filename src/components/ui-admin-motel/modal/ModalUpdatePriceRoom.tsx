...
export const ModalUpdatePriceRoom = ({ isOpen, onClose, selectedRooms, onRemoveRoom }: ModalProps) => {


    const [isLoadingUpdatePrice, setIsLoadingUpdatePrice] = useState(false);
    const [newPriceRoom, setNewPriceRoom] = useState("");
    ...
    const onUpdatePrice = async () => {
        ...
        const newPrice = Number(newPriceRoom);

        if (isNaN(newPrice)) {
            setErrorInput(true);
            setMessageError("El precio debe ser un número válido");
        ...
        if (newPrice < 10000) {
            setErrorInput(true);
            setMessageError("El precio no debe ser un valor menor a $10,000");
            setIsLoadingUpdatePrice(false);
            return;
        }
        ...
        if (!newPrice) {
            setErrorInput(true);
            setMessageError("El precio es obligatorio");
            setIsLoadingUpdatePrice(false);
            return;
        }
        if (newPrice < 10000) {
            ...
            >
                <div className="bg-white md:rounded-lg shadow-lg w-full md:w-6/12 max-h-full overflow-y-auto custom-scrollbar">
                    ...
                    <div className="py-3 px-4 shadow-md flex justify-between items-center border-b border-gray-200">
                        <p className="text-lg font-semibold">Actualizar precio</p>
                    ...
                    <div className="grid grid-cols-2 px-6 gap-4 py-4 mt-2">
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                onChange={(e) => setNewPriceRoom(e.target.value)}
                            ...
                            <div className="flex mt-3 justify-start gap-2 md:gap-0  sm:flex-row sm:space-x-3 sm:space-y-0">
